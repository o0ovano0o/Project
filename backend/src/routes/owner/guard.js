...
router.post('/api/owner/guard', validateOwnerAPI, async(req, res) => {
    try {
        ...
        if (!result) return res.status(400).json({ success: true, msg: 'Đăng ký tài khoản bảo vệ thất bại' });
...
router.delete('/api/owner/guard/:userid',validateOwnerAPI, async(req, res) => {
    try {
        ...
        const ownerid = req.session.userid;
        if (!userid || !ownerid) return res.status(400).json({ success: false, msg: 'Thông tin bắt buộc bị thiếu' });
        const check = await knex('user')
            .delete()
            .where({ userid, ownerid });
        if (!check) return res.status(400).json({ success: false, msg: 'Xóa tài khoản bảo vệ thất bại' });
...
router.put('/api/owner/guard/:userid', validateOwnerAPI, async(req, res) => {
    try {
        ...
        if (!result) return res.status(400).json({ success: true, msg: 'Sửa thông tin tài khoản bảo vệ thất bại' });
...
router.get('/api/owner/guards',validateOwnerAPI, async(req, res) => {
    try {
        const ownerid = req.session.userid;
        const guards = await knex('user').select().where({ ownerid }).orderBy('userid', "desc");
        return res.status(200).json({
            success: true,
            data: guards,
        });
...
router.get('/api/owner/guard/:id',validateOwnerAPI, async(req, res) => {
    try {
        ...
        if (!guard) return res.status(400).json({ success: false, msg: 'Lấy thông tin bải vệ thất bại' });
        return res.status(200).json({
            success: true,
            data: guard,
        });
